/*
 * Duty Booking API and Events
 *
 * ## Duty Booking API and events and Events 
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: support@allocatesoftware.com
 * Generated by: https://openapi-generator.tech
 */

using System;
using System.Linq;
using System.Text;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using ASW.APIServices.Core.Converters;

namespace ASW.APIServices.Core.Models
{ 
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class DutyWhen : IEquatable<DutyWhen>
    { 
        /// <summary>
        /// Gets or Sets Planned
        /// </summary>
        [DataMember(Name="planned", EmitDefaultValue=false)]
        public DutyTimes Planned { get; set; }

        /// <summary>
        /// Gets or Sets Actual
        /// </summary>
        [DataMember(Name="actual", EmitDefaultValue=false)]
        public DutyTimes Actual { get; set; }

        /// <summary>
        /// Night, Day, etc
        /// </summary>
        /// <value>Night, Day, etc</value>
        [DataMember(Name="shiftType", EmitDefaultValue=false)]
        public string ShiftType { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class DutyWhen {\n");
            sb.Append("  Planned: ").Append(Planned).Append("\n");
            sb.Append("  Actual: ").Append(Actual).Append("\n");
            sb.Append("  ShiftType: ").Append(ShiftType).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (obj is null) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((DutyWhen)obj);
        }

        /// <summary>
        /// Returns true if DutyWhen instances are equal
        /// </summary>
        /// <param name="other">Instance of DutyWhen to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(DutyWhen other)
        {
            if (other is null) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    Planned == other.Planned ||
                    Planned != null &&
                    Planned.Equals(other.Planned)
                ) && 
                (
                    Actual == other.Actual ||
                    Actual != null &&
                    Actual.Equals(other.Actual)
                ) && 
                (
                    ShiftType == other.ShiftType ||
                    ShiftType != null &&
                    ShiftType.Equals(other.ShiftType)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (Planned != null)
                    hashCode = hashCode * 59 + Planned.GetHashCode();
                    if (Actual != null)
                    hashCode = hashCode * 59 + Actual.GetHashCode();
                    if (ShiftType != null)
                    hashCode = hashCode * 59 + ShiftType.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(DutyWhen left, DutyWhen right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(DutyWhen left, DutyWhen right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
